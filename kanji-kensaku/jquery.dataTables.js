	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			sSearchStr = (sSearchStr.indexOf('_INPUT_') !== -1) ?
			  sSearchStr.replace('_INPUT_', '<input type="text" id="bunkjpSearchInput" />') :
			  sSearchStr==="" ? '<input type="text" id="searchInput" />' : sSearchStr+' <input type="text" id="searchInput" />';
			...
			jqFilter.bind( 'keyup.DT', function(e) {
				...
				for ( var i=0, iLen=n.length ; i<iLen ; i++ )
				{
					...
					{
						// $('input', n[i]).val( this.value ).addClass('okinput').on('click',()=>{alert('okinput')});
